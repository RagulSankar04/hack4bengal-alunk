...
const Amount = ({ scanedResult, setScanedResult }) => {
  ...
  const pay = () => {
    ...
    setDoc(
      doc(db, "users", payer?.uid),
      {
        balance: parseInt(payer.balance) - parseInt(amount),
      },
      { merge: true }
    )
      .then(() => {
      ...
      .catch(alert);
    setDoc(
      doc(db, "users", payee?.uid),
      {
        balance: parseInt(payee.balance) + parseInt(amount),
      },
      { merge: true }
    )
      .then(() => {
      ...
      <div className="flex flex-col gap-4 justify-center items-center w-full h-full">
        ...
        <div className="flex flex-col gap-4">
          <input
            ...
            type="number"
            className="appearance-none border rounded w-fit py-2 px-3 h-12 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-[#008037]"
          />
          <input
            value={note}
            placeholder="Note"
          ...
          >
            <span className="text-normal font-light">Next</span>
          </button>
        </div>
      </div>
    </div>
